...
class ClientMasterServicesScreen extends StatefulWidget {
  final int? id;
  final List<int?> chosenServices;

  const ClientMasterServicesScreen({
    super.key,
    required this.id,
  ...
  @override
  State<ClientMasterServicesScreen> createState() =>
      _ClientMasterServicesScreenState();
}

class _ClientMasterServicesScreenState
    extends State<ClientMasterServicesScreen> {
  final infoController = GetIt.instance<ClientMasterServicesController>();
  final controller = GetIt.instance<ClientMasterServicesStateController>();
          ...
          if (!infoController.stateManager.isSuccess) {
            return StateControlWidget(
              props: StateControlWidgetProps(
                ...
                onError: () {
                  infoController.fetchMasterInfo(widget.id ?? 0, () {
                    controller.initData(infoController.data, context, widget.chosenServices);
                  });
                },
                // isSliver: true,
              ),
